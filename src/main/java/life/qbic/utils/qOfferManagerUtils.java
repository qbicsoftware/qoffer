...
//          + "/WEB-INF/resourceFiles/Credentials.properties";

  private static final Logger LOG = LogManager.getLogger(qOfferManagerUtils.class);
  ...
  public static String formatCurrency(String currencyToFormat) {
    try {
      ...
      currencyToFormat = format.format(new BigDecimal(currencyToFormat));
    } catch (Exception e) {
      LOG.warn("could not format currency string for "+currencyToFormat);
   ...
   * @param type: one of "error", "success" and "warning". Changes the style and the delay of the
   *        displayNotification.
   */
  public static void displayNotification(String title, String description, String type) {
    ...
    // make sure that, if we are running this from a background thread, we are accessing the UI in a
    // thread-safe way
  ...
  private static String getContainerContents(SQLContainer container, String delimiter) {
    ...
    containerContents = new StringBuilder(
        containerContents.substring(0, containerContents.length() - delimiter.length()));
    ...
    for (Object rowId : container.getItemIds()) {
      ...
      for (Object columnId : container.getContainerPropertyIds()) {
        try {
          containerContents
              .append(
                  container.getContainerProperty(rowId, columnId.toString()).getValue().toString())
              .append(delimiter);
